...
try:
    ...
    from ..api.routers.transcripts import _resolve_notes_transcript, generate_notes_with_gemini_background
...
async def _generate_meeting_notes_async(
    ...
    db = DatabaseManager()
    logger.info(f"[Task:GenerateNotes] Starting for meeting={meeting_id} user={user_email}")
    ...
    try:
        ...
        if not full_transcript_text.strip() and source == "recall_bot":
            logger.info(f"[Task:GenerateNotes] Empty transcript for bot meeting {meeting_id}. Attempting Recall API recovery...")
            try:
            ...
            if not full_transcript_text.strip():
                ...
                if retries < max_retries:
                    logger.info(f"[Task:GenerateNotes] Transcript still empty for {meeting_id} (Attempt {retries+1}/{max_retries}). Retrying...")
                    raise Exception(f"Transcript not ready for bot meeting {meeting_id}")
                else:
                    logger.warning(f"[Task:GenerateNotes] Max retries reached for {meeting_id}. Skipping.")
                    return {"status": "skipped", "reason": "empty_transcript_after_retries"}

        if not full_transcript_text.strip():
            logger.warning(f"[Task:GenerateNotes] Empty transcript for meeting={meeting_id}. Skipping.")
        ...
        try:
            ...
            from app.api.routers.transcripts import generate_notes_with_gemini_background
        ...
        logger.info(f"[Task:GenerateNotes] Successfully completed for meeting={meeting_id}")
